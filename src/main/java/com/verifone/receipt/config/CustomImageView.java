...
public class CustomImageView extends View {
     ...
     * <p>
     * This is set to false as swing does not offer a repaint that takes a delay. If
     * this were true, a bunch of immediate repaints would get generated that end up
     * significantly delaying the loading of the image (or anything else going on
     * for that matter).
     */
    ...
    /**
     * Returns the text to display if the image can't be loaded. This is obtained
    ...
    /**
     * Sets how the image is loaded. If <code>newValue</code> is true, the image we
     * be loaded when first asked for, otherwise it will be loaded asynchronously.
     * The default is to not load synchronously, that is to load the image
     * asynchronously.
     */
     ...
     */
    public synchronized boolean getLoadsSynchronously() {
    ...
    protected void setPropertiesFromAttributes() {
        ...
        Object alignment = attributeSet.getAttribute(HTML.Attribute.ALIGN);
    ...
    private void iconImage(Graphics g, Rectangle rect) {
    ...
    public float getPreferredSpan(int axis) {
        ...
        if (axis == View.X_AXIS && (state & WIDTH_FLAG) == WIDTH_FLAG) {
            ...
            return width + leftInset + (float) rightInset;
        }
        if (axis == View.Y_AXIS && (state & HEIGHT_FLAG) == HEIGHT_FLAG) {
            getPreferredSpanFromAltView(axis);
            return height + topInset + (float) bottomInset;
        }
    ...
    private int getIntAttr(HTML.Attribute name, int deflt) {
        ...
        if (attributes.isDefined(name)) { // does not check parents!
            int i;
            String val = (String) attributes.getAttribute(name);
            if (val == null) {
                i = deflt;
            } else {
                try {
    ...
    private void updateImageSize() {
        ...
        if (newImage != null) {
            ...
            newHeight = getNewHeight(newWidth, newHeight, newState, newImage);

            boolean createText = false;
            createText = isCreateText(newWidth, newHeight, newState, createText);
            if (createText) {
    ...
    private int getNewHeight(int newWidth, int newHeight, int newState, Image newImage) {
        if (newHeight <= 0) {
    ...
    private boolean isCreateText(int newWidth, int newHeight, int newState, boolean createText) {
        synchronized (this) {
    ...
    private class ImageHandler implements ImageObserver {
        ...
        public boolean imageUpdate(Image img, int flags, int x, int y, int newWidth, int newHeight) {
            ...
            if ((flags & (ABORT | ERROR)) != 0) {
                repaint(0);
                if (enableOrDisableImage(img)) return false;
                updateAltTextView();
            ...
            if (image == img) {
                ...
                short changed = 0;
                changed = imageObserverChanges(flags, changed);

                if (imageResize(newWidth, newHeight, changed)) return true;
                if (changed != 0) {
        ...
        private short imageObserverChanges(int flags, short changed) {
        ...
        private boolean enableOrDisableImage(Image img) {
        ...
        private boolean imageResize(int newWidth, int newHeight, short changed) {
            synchronized (CustomImageView.this) {
                if ((changed & 1) == 1 && (state & WIDTH_FLAG) == 0) {
                    width = newWidth;
                }
                if ((changed & TWO) == TWO && (state & HEIGHT_FLAG) == 0) {
                    height = newHeight;
                }
                if ((state & LOADING_FLAG) == LOADING_FLAG) {
    ...
    private class ImageLabelView extends InlineView {
        ...
        public Segment getText(int p0, int p1) {
            if (p0 < 0 || p1 > segment.array.length) {
                try {
                    throw new ImagingOpException("ImageLabelView: Stale view");
        ...
        public Color getForeground() {
            ...
            if (fg == null && (parent = getParent()) != null) {
                ...
                if (attr != null && (doc instanceof StyledDocument)) {
                    fg = ((StyledDocument) doc).getForeground(attributeSet);
                }
            }
            return fg;
        }
    ...
    private void loadImage() {
        ...
        if (src != null) {
            ...
            try (ByteArrayInputStream bais = new ByteArrayInputStream(DatatypeConverter.parseBase64Binary(b64))) {
                ...
                log.error("[ERROR] in loadImage() : {} ", ex);
            }
